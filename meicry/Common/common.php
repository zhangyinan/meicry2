 ...
 * @param Int     $timeout
 */
function xcurl($url, $ref = null, $post = array(), $timeout = 60, $ua = "Mozilla/5.0 (X11; Linux x86_64; rv:2.2a1pre) Gecko/20110324 Firefox/4.2a1pre", $print = false) {
    $ch           = curl_init();
    ...
    $output       = curl_exec($ch);
    if ($output === false) {
        Log::write("Curl failed, Error info: ".curl_error($ch));
    }
 ...
 * @param Int     $timeout
 */
function curlPost($url, $post, $timeout = 2) {
    $ch       = curl_init();
